import sqlite3
import csv
import smtplib
from email.mime.text import MIMEText
from email.message import EmailMessage

#create a DB
db = sqlite3.connect("DB_Proj.sqlite")

# create a Table
try:
    db.execute('''create table student(
                    Roll Integer,
                    Name text,
                    Email text,
                    Math Integer,
                    Science Integer,
                    Computer Integer)''')
except:
    pass

def addStudent():
    print("----- Create a new Student Profile ----")

    #list1 = ["Roll No","Name","Email Address","Enter marks in Maths","Enter marks in science","Enter marks in computer"]
    list2 = []
    roll = input("Enter Roll No.: ")
    while roll.isdigit() != True:
        print("Invalid name! Please try again")
        roll = input("Enter Roll.no.: ")
    list2.append(roll)

    n = input("Enter name: ")
    while n.isalpha() == False:
        print("Invalid name! Please try again")
        n = input("Enter name:")
    list2.append(n)

    email = input("Enter Your Email address: ")
    list2.append(email)
    m_m = int(input("Enter marks in maths:"))
    list2.append(m_m)
    m_s = int(input("Enter makrs in Science: "))
    list2.append(m_s)
    m_comp = int(input("Enter marks in Computer: "))
    list2.append(m_comp)

    db.execute("insert into student values(?,?,?,?,?,?)",list2)
    db.commit()
    print("-*- Student Information Added Successfully -*-")

def checkResult():
    print("------ Check Your Result Here ------")
    cursor = db.cursor()
    r = int(input("Enter you roll number to fetch you Marks:"))
    cursor.execute("Select * from student where roll =?",[r,])
    var = cursor.fetchall()
         
    print("Roll Number:",var[0][0])
    print("Name:",var[0][1])
    print("Email Id:",var[0][2])
    print("Math:",var[0][3])
    print("Science:",var[0][4])
    print("Computer:",var[0][5])

    perc = (var[0][3]+var[0][4]+var[0][5])/3
    print("Percentage: %.2f"%perc)
    print("*-"*30)
    return perc

def genReport():
    cursor = db.cursor()
    cursor.execute("select * from student")
    var = cursor.fetchall()
    print(var)

    #writing this data into a CSV file

    f = open("report.csv","w",newline="")
    obj = csv.writer(f)
    obj.writerow(["Roll No.","Student Name","Email Id","Maths","Science","Computer","Percentage","Status"])
    

    for i in var:
        perc = "%.2f" %((i[-1]+i[-2]+i[-3])/3)
        if i[-1]>35 and i[-2]>35 and i[-3]>35:
            status = "PASS"
        else:
            status = "FAIL"

        list2 = list(i)
        list2.append(perc)
        list2.append(status)

        obj.writerow(list2)

    f.close()
    print("---- Report Generated Successfully ----")

def getMessage(math, science, computer, percent):
    return f"""From:HALF YEARLY EXAM
            Subject: Result of Half-Yearly Exam 2022
            Computer= {computer}
            Math = {math}
            Science = {science}
            Percentage scored = {percent}"""



def sendEmail(sender, password, message, destination):

    server = smtplib.SMTP('smtp.gmail.com', 587)
    server.starttls()

    server.login(sender, password)
    msg = EmailMessage()
    msg.set_content(message)

    msg['Subject'] = 'Result'
    msg['From'] = sender
    msg['To'] = destination
    server.send_message(msg)
    

def sendReport():
    sender = input("Enter Login Id:")
    password = input("Enter  log in Password:")
    with open('report.csv', mode ='r')as file:

        msgReader = csv.reader(file)
        for [Roll_No, Student, Email_Id, Maths, Science, Computer, Percentage, Status] in list(msgReader)[1:]:
            message = getMessage(Maths, Science, Computer, Percentage)
            sendEmail(sender, password, message, Email_Id)

    print("-- Mail Sent Succcessfully --")


while True:
    print("***** MAIN MENU ********")
    print("1. Add a Student")
    print("2. Check Percentage")
    print("3. Convert into CSV file")
    print("4. Mail Report ")
    print("5. Exit")
    ch = int(input("Enter your Choice:"))
    if ch == 1:
        addStudent()
    elif ch == 2:
        checkResult()
    elif ch == 3:
        genReport()
    elif ch == 4:
        sendReport()
    elif ch == 5:
        print("--- Successfully Done ---")
        break
    else:
        print("--- Input the correct values ---")
